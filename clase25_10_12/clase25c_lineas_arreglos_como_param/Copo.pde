...
class Copo {
  ...
  Copo() {  //constructor SIN parametros
    ...
    y = random( -100, -1000 );
    tamanio = random(10, 50);  //lo defino mediante parámetro del constructor
    velocidad = random( 2, 5 );
  ...
  //------------------------------------------------------------------------------------------
  void drawLines( Copo[] c ){
    for( int i = 0 ; i < c.length ; i++ ){
      float distancia = dist( x, y, c[i].getX(), c[i].getY() );
      float sumaDeRadios = getRadio() + c[i].getRadio();
      if( distancia < sumaDeRadios ){  //en criollo: si las circunferencias 'se tocan' -> noción de colisión
        stroke(0, map(distancia, 0, sumaDeRadios, 0, 255 ) );
        line( x, y, c[i].getX(), c[i].getY() );       
      }
    }
  }
  float getX(){  //simple pero importante para ACCEDER a los CAMPOS de una CLASE
    return x;
  }
  float getY(){
    return y;
